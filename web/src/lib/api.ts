...
export const api = {
  ...
  adminResetPassword: (id: string) => request<{ tempPassword: string }>(`/admin/users/${id}/reset-password`, { method: 'POST' }),
  ...
  createAssessment: (form: FormData) => request<{ assessment: any }>('/assessments', { method: 'POST', body: form, isForm: true }),
  updateAssessment: (id: string, data: { notes?: string }) => request<{ assessment: any }>(`/assessments/${id}`, { method: 'PATCH', body: data }),
  ...
  getSystemMetrics: () =>
    request<{ assessmentCount: number; imageCount: number; storageBytes: number; lastCalculated: number }>('/metrics/system'),
};
